...
function setup(){
  ...
  Jeorge = createSprite(50,100,20,20);
  ...
  chao2 = createSprite (300,200,600,20);
  chao2.visible = false;
...
function draw(){
  ...
  if (modo === JOGAR){
    ...
    if((keyDown("space")||keyDown("up"))&& Jeorge.isTouching(chao)){
      Jeorge.velocityY = -14;
...
function gerar_nuvens () {
 ...
 if (frameCount %60 === 0) {
  ...
  nuvens = createSprite (590,100,10,10);
  nuvens.velocityX = -5;
...
function gerar_cactos () {
  ...
  if (frameCount %60 === 0) {
   ...
   cacto = createSprite (620, 175,10,10);
